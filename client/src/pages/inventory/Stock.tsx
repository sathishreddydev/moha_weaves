...
export default function InventoryStock() {
  ...
  const [selectedSaree, setSelectedSaree] = useState<SareeWithDetails | null>(
    null
  );
  const [stockForm, setStockForm] = useState({
    totalStock: 0,
    onlineStock: 0,
    distributionChannel: "both" as "shop" | "online" | "both",
  });
  ...
  const updateStockMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await apiRequest(
        "PATCH",
        `/api/inventory/sarees/${sareeId}/stock`,
        { totalStock, onlineStock }
      );
  ...
  const updateDistributionMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await apiRequest(
        "PATCH",
        `/api/inventory/sarees/${sareeId}/distribution`,
        { channel }
      );
      return response.json();
    },
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Update inventory levels and distribution channels
          </p>
        </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredSarees?.map((saree) => (
                      <TableRow
                        key={saree.id}
                        data-testid={`row-stock-${saree.id}`}
                      >
                        <TableCell>
                        ...
                        <TableCell>
                          ...
                          >
                            <SelectTrigger
                              className="w-32"
                              data-testid={`select-channel-${saree.id}`}
                            >
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{selectedSaree?.name}</DialogDescription>
          </DialogHeader>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setStockForm({
                      ...stockForm,
                      onlineStock: Math.min(
                        parseInt(e.target.value) || 0,
                        stockForm.totalStock
                      ),
                    })
                  }
