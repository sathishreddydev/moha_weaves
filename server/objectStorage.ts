...
const MOHASIDECAR_ENDPOINT = "http://127.0.0.1:1106";
...
export const objectStorageClient = new Storage({
  credentials: {
    ...
    credential_source: {
      url: `${MOHASIDECAR_ENDPOINT}/credential`,
      format: {
        type: "json",
...
export class ObjectStorageService {
  ...
  async getObjectEntityUploadURL(fileType: string = "image"): Promise<{ uploadURL: string; objectPath: string; uploadToken: string }> {
    ...
    return { uploadURL, objectPath, uploadToken };
  }

  verifyUploadToken(uploadToken: string, objectPath: string): boolean {
    ...
    if (pending.objectPath !== objectPath) {
      return false;
    }
    pendingUploads.delete(uploadToken);
    return true;
  ...
  isValidObjectPath(path: string): boolean {
