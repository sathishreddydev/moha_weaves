...
SELECT SUM(quantity) FROM store_inventory WHERE product_id = ${products.id}
  ...
  > {
    ...
    if (filters?.page && filters?.pageSize) {
      ...
      // Get paginated data
      const data = await query.limit(filters.pageSize).offset(offset);
  ...
  > {
    ...
    const turnoverData = currentStock.map((product) => {
      ...
      const turnoverRatio =
        averageStock > 0 ? costOfGoodsSold / averageStock : 0;

      const daysOfSupply =
        quantitySold > 0 ? (product.totalStock / quantitySold) * 365 : 999;
  ...
  > {
    ...
    const salesData = await db
      ...
      .having(
        sql`ABS(SUM(CASE WHEN movement_type = 'sale' THEN quantity ELSE 0 END)) > 0`,
      );
  ...
  > {
    ...
    const productGroups = monthlySales.reduce(
      (groups, sale) => {
    ...
    const trendsData = Object.values(productGroups).map((product: any) => {
      ...
      const denominator = xValues.reduce(
        (sum: number, x: number) => sum + Math.pow(x - xMean, 2),
        0,
      );
      ...
      monthlyData.forEach((data: any) => {
        ...
        monthlyAverages.set(
          monthName,
          monthlyAverages.get(monthName)! + data.quantity,
        );
      });
      ...
      const avgMonthly =
        Array.from(monthlyAverages.values()).reduce((a, b) => a + b, 0) / 12;

      const variance =
        Array.from(monthlyAverages.values()).reduce(
          (sum, val) => sum + Math.pow(val - avgMonthly, 2),
          0,
        ) / 12;
